...
def get_radar_odm_samples(data_loc,frames,smp_path,opt_path,clip,split, label_path, mode):
    ...
    for i in tqdm(range(num_frames-2),desc="generate scene flow samples for " + clip):
        get_one_sample(frames[i], frames[i+1], frames[i+2], data_loc, save_path, opt_path, label_path, mode, clip)


def get_one_sample(frame1, frame2, frame3, data_loc, save_path, opt_path, label_path, mode, clip):
    ...
    data2 = FrameDataLoader(kitti_locations=data_loc,
                    frame_number=frame2)
    data3 = FrameDataLoader(kitti_locations=data_loc,
                    frame_number=frame3)
    ...
    transforms3 = FrameTransformMatrix(data3)

    # x y z RCS v_r
    radar_data1 = data1.radar_data[:,0:5]
    radar_data2 = data2.radar_data[:,0:5]
    # draw_multi_lidar([radar_data1, radar_data2])
    ...
    if not (radar_data1.shape[0]<min_pnts or radar_data2.shape[0]<min_pnts):
        ...
        odom_cam_2 = transforms2.t_odom_camera
        odom_cam_3 = transforms3.t_odom_camera
        cam_radar_2 = transforms2.t_camera_radar
        cam_radar_3 = transforms3.t_camera_radar
        odom_radar_2 = np.dot(odom_cam_2,cam_radar_2)
        odom_radar_3 = np.dot(odom_cam_3,cam_radar_3)
        radar2_radar3 = np.dot(np.linalg.inv(odom_radar_2), odom_radar_3)
        ...
        labels3 = load_track_labels(label_path, frame3, mode)

        gt_mask1, gt_label1 = process_label(labels1, labels2, radar_data1, transforms1, transforms2, radar1_radar2, 'radar')
        gt_mask2, gt_label2 = process_label(labels2, labels3, radar_data2, transforms2, transforms3, radar2_radar3, 'radar')
        # draw_single_lidar_with_label(radar_data1, gt_mask1.astype('int'))
        # draw_single_lidar_with_label(radar_data2, gt_mask2.astype('int'))

        # radar_data1 = radar_data1[gt_mask1 == 1]
        # radar_data2 = radar_data2[gt_mask2 == 1]
        ...
        gt_mask1 = gt_mask1.tolist()
        gt_label1 = gt_label1.tolist()
        gt_mask2 = gt_mask2.tolist()
        gt_label2 = gt_label2.tolist()
...
def process_label(labels1, labels2, radar_data1, transforms1, transforms2, radar1_radar2, model):

    fg_idx, fg_confs, fg_labels, fg_bboxes = extract_fg_labels(labels1, labels2, radar_data1, transforms1, transforms2, model)

    gt_mask = np.zeros(radar_data1.shape[0], dtype=np.float32)  # moving mask
    gt_labels = np.zeros((radar_data1.shape[0], 3), dtype=np.float32)  # scene flow label
