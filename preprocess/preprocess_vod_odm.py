...
from utils.get_odm_samples import get_radar_odm_samples
...
def main(args):
    ...
    smp_path = os.path.join(save_dir, 'odm_smp')
    ...
    for split in splits:
        for clip in splits[split]:
            ...
            if split == 'train':
                get_radar_odm_samples(data_loc,frames,smp_path,opt_path, clip, split, true_label_path, mode='train')
            if split == 'val':
                get_radar_odm_samples(data_loc,frames,smp_path,opt_path, clip, split, true_label_path, mode='val')
            if split == 'test':
                get_radar_odm_samples(data_loc,frames,smp_path,opt_path, clip, split, true_label_path, mode='test')
